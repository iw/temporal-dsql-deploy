...
Usage:
    uv run python dsql-tests/copilot/error_injection.py
    uv run python dsql-tests/copilot/error_injection.py --failure-pct 30 --rate 15
    uv run python dsql-tests/copilot/error_injection.py --mode timeout --failure-pct 50
"""
...
class FailureMode(str, Enum):
    EXCEPTION = "exception"  # Activity raises ApplicationError
    TIMEOUT = "timeout"  # Activity exceeds start-to-close timeout
    MIXED = "mixed"  # Random mix of both
...
@activity.defn
async def flaky_work(params: dict) -> str:
    ...
    if params.get("should_fail"):
        ...
        if mode == "timeout":
            # Sleep longer than the start-to-close timeout
            await asyncio.sleep(120)
            return "unreachable"
        else:
            # Simulate a non-retryable application error
...
class ErrorInjectionWorkflow:
    ...
    async def run(self, params: dict) -> str:
        # Step 1: always succeeds (generates a state transition)
        ...
        # Step 2: may fail (the injection point)
...
async def main() -> None:
    ...
    parser.add_argument("--metrics-port", type=int, default=9091, help="Prometheus port (default: 9091)")
    ...
    async def run_one(n: int) -> None:
        async with semaphore:
            should_fail = random.randint(1, 100) <= args.failure_pct

            if args.mode == "mixed":
                failure_mode = random.choice(["exception", "timeout"])
            else:
